...
public class AdminRoomController : Controller
{
    ...
    public async Task<IActionResult> AddRoom(CreateRoomDto createRoomDto)
    {
        ...
        var responseMessage = await client.PostAsync("http://localhost:5027/api/Room", stringContent);
        if (responseMessage.IsSuccessStatusCode)
        {
    ...
    public async Task<IActionResult> UpdateRoom(UpdateRoomDto updateRoomDto)
    {
        ...
        StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
        var responseMessage = await client.PutAsync("http://localhost:5027/api/Room/", stringContent);
        if (responseMessage.IsSuccessStatusCode)
        {
            return RedirectToAction("Index");
        }
